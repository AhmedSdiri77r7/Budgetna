    ...
    <table class="table">
      ...
      <tbody>
        <tr *ngFor="let budget of budgets">
          ...
          <td [ngClass]="budget.valide === 'true' ? 'red' : 'green'">{{ budget.valide }}</td>
          <td>
            <button class="btn btn-outlin-info" (click)="opendet()">Details</button>
          </td>
        </tr>
      </tbody>
    </table>
...
@Component({
  selector: 'ngx-tab2',
  template: ` <!-- Add your Tab2 component template here --> `,
})
...
export class TabsComponent {
  ...
  getBudgets() {
    this.budgetService
      .getBudgetsByBudgetInitialAndDate(
        this.idEmploye.toString(),
        this.idBudgetInitial.toString(),
